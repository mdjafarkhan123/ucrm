-- Part 5D fix: the attachment copy's unqualified `order by position, id` collided with the SELECT list's
-- own `new_quote.id` / `new_version.id` output columns ("ORDER BY \"id\" is ambiguous"). Aliasing the
-- source table, like the line copy already does, removes the ambiguity.
