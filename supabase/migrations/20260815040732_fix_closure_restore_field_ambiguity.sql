-- Fix: apply_organization_closure_restore's UPDATE ... SET referenced the parameter
-- restoration_evidence_note where Postgres also sees the identically named column on the target
-- table in scope, causing "column reference is ambiguous". Same class of bug as the 6D
-- fix_free_access_change_record_field_access migration -- qualify the parameter with the function
-- name wherever it collides with a column name.
