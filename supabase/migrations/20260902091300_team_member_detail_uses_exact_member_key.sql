-- The detail route has an exact organization/member key. `status <> 'removed'` is equivalent to the
-- established four-state constraint here, but lets Postgres use the membership primary key instead of
-- scanning every current-status member in an organization before matching the requested person.
