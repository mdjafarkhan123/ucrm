-- Follow-up fix: extending (or converting to forever) the currently active free access grant did
-- not check for overlap against an already-scheduled future grant, letting the active grant's new
-- end date silently swallow the future grant's start. Add the missing overlap check.
