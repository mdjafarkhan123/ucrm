-- Saving a team member's details died on every call: `changed_fields || 'full_name'` let Postgres read the
-- bare literal as another text[], so it tried to parse "full_name" as an array and raised 22P02 before any
-- detail was ever written. Casting the literal makes it the append it was always meant to be.
