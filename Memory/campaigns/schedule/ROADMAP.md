...
| 7b | V1.2 | Live managed-Mapbox integration + verification | Blocked — needs Jafar's Mapbox tokens | Part 7a; Mapbox account + server secret token (Permanent geocoding) + browser public token | Real Mapbox Permanent geocoding with STORED property coordinates + one-time rate-limited backfill, live tile/numbered/grouped-marker/route-line rendering, external navigation confirmed, rate-limit + provider-error behavior verified live; full V1.2 completion journey passes (open Map for one employee, understand stops, order Anytime, save, handle unmappable address, open Directions). Re-verify Mapbox pricing/storage terms before any paid commitment |
