...
function event(id: string, body: unknown) {
	return {
		...
		request: new Request('http://localhost/api/jafar/operations/' + id + '/resolve', {
			method: 'POST',
			body: JSON.stringify(body)
		}),
		url: new URL('http://localhost/api/jafar/operations/' + id + '/resolve'),
		cookies: {}
...
describe('platform owner operation resolve API boundary', () => {
	...
	it('resolves an open operation with the given note', async () => {
		...
		expect(client.__update).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				resolved_by_owner_email: 'owner@example.com',
	...
	it('returns a safe server error when the update fails', async () => {
		...
		expect(await response.json()).toEqual({ error: 'The operation could not be resolved.' });
	});
});
