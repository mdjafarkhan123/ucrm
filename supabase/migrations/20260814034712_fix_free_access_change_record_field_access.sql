-- Follow-up fix: PL/pgSQL cannot reference a field on a generic `record` variable in the same
-- boolean expression as its `is not null` guard, because an unassigned record has indeterminate
-- tuple structure until the whole expression is planned. Split into nested IF statements.
