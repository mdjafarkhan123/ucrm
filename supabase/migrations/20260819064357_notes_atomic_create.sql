-- Bug fix: `POST /api/notes` inserted the note and its note_links row as two separate `supabase-js` calls.
-- If the second insert failed (a stale entity, a lost permission, a dropped connection), the first was
-- already committed and the note was orphaned -- it exists, is unlinked from everything, and nothing ever
-- shows or cleans it up. This wraps both inserts in one function call, so a failure partway through rolls
-- back the whole thing. RLS still applies to both inserts (security invoker): this fixes atomicity, not
-- who may write.
