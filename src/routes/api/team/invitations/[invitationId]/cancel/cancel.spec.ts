...
import { cancelTeamInvitation, TeamInvitationError } from '$lib/server/team/invitations';
...
vi.mock('$lib/server/team/invitations', async () => {
	...
	return { ...actual, cancelTeamInvitation: vi.fn() };
});
...
function event(id = invitationId) {
	return {
		request: new Request(`https://app.example.com/api/team/invitations/${id}/cancel`, {
			method: 'POST'
		}),
		url: new URL(`https://app.example.com/api/team/invitations/${id}/cancel`),
		params: { invitationId: id },
		locals: {}
	} as Parameters<typeof POST>[0];
}

describe('team invitation cancel API', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		...
		vi.mocked(cancelTeamInvitation).mockResolvedValue({ invitationId, status: 'cancelled' });
	});
	...
	it('authorizes, rate limits, and returns a no-store success', async () => {
		...
		expect(checkRateLimit).toHaveBeenCalledWith(client, {
			bucketKey: `team_invitation_cancel:${organizationId}:${managerId}`,
			windowSeconds: 300,
			maxAttempts: 20
		});
		expect(cancelTeamInvitation).toHaveBeenCalledWith(client, {
			organizationId,
			invitationId,
	...
	it('rate limits before cancellation', async () => {
		...
		expect(response.headers.get('cache-control')).toBe('no-store');
		expect(cancelTeamInvitation).not.toHaveBeenCalled();
	});

	it('returns a stable conflict while acceptance is in progress', async () => {
		vi.mocked(cancelTeamInvitation).mockRejectedValueOnce(
