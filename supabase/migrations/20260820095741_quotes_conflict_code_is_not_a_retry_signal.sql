-- Quotes, Part 2: the stale-conflict error code that made a refused save hang forever.
--
-- Both write commands refused a conflict by raising SQLSTATE 40001. That is Postgres' serialization
-- failure -- the standard signal for "two transactions collided, the caller should simply try again".
-- PostgREST reads it exactly that way and retries the call itself. Our two uses are not transient at all:
-- a stale revision stays stale, and a request that already has a quote still has one. So the retry raised
-- the same error, and the request never came back at all.
--
-- Measured against one request, back to back: a save with the right revision answered in 334 ms, a save
-- with a stale revision never answered (still pending at 35 s), and a save that failed an ordinary
-- check_violation answered in 447 ms. Calling the function directly returned the stale error in 0.78 s,
-- so the database was never the slow part. In the app the editor sat on a spinning Save button with no
-- message and no way out but a page reload, and each abandoned attempt left a half-finished transaction
-- holding the request row's lock, which slowed every later save on that request.
--
-- P0409 replaces it in both functions: a code of our own that nothing retries, mapped in
-- src/lib/server/quotes/errors.ts to the same 409 responses these conflicts already produced. Nothing
