...
import { quoteTargetMarginSchema } from '$lib/server/validation/settings.schema';
...
export const PATCH: RequestHandler = async (event) => {
	...
	try {
		limit = await checkRateLimit(event.locals.supabase, {
			bucketKey: `settings-quotes-target-margin:${organizationId}`,
			...SAVE_LIMIT
		});
	} catch {
	...
	const parsed = quoteTargetMarginSchema.safeParse(body);
	if (!parsed.success) return validationError(zodFieldErrors(parsed.error));

	const { data, error } = await event.locals.supabase.rpc('set_organization_quote_target_margin', {
		target_organization_id: organizationId,
		expected_revision: parsed.data.expected_revision,
		new_margin_basis_points: parsed.data.margin_basis_points
	});
