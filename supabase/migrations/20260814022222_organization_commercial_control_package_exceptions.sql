-- Part 6C: immediate versioned package changes and reasoned access exceptions.
-- New writes use the commercial command seam. Existing override rows remain effective as
-- explicitly marked legacy imports until a Platform Owner reviews them.
...
alter table public.organization_commercial_events
  ...
  add constraint organization_commercial_events_change_payload_check check (
    (change_before is null or jsonb_typeof(change_before) = 'object')
    and (change_after is null or jsonb_typeof(change_after) = 'object')
  );

alter table public.organization_feature_overrides
  add column if not exists reason text,
  add column if not exists actor_owner_email text,
  add column if not exists is_legacy_import boolean not null default true;

alter table public.organization_feature_overrides
  add constraint organization_feature_overrides_reason_check check (
    is_legacy_import or char_length(trim(coalesce(reason, ''))) between 1 and 1000
  ),
  add constraint organization_feature_overrides_actor_email_check check (
...
alter table public.organization_limit_overrides
  add column if not exists limit_state text not null default 'numeric',
  add column if not exists reason text,
  add column if not exists actor_owner_email text,
  add column if not exists is_legacy_import boolean not null default true;

update public.organization_limit_overrides
set limit_state = case when is_unlimited then 'unlimited' else 'numeric' end
where limit_state = 'numeric';

alter table public.organization_limit_overrides
  drop constraint if exists organization_limit_overrides_value_state_check;

alter table public.organization_limit_overrides
  add constraint organization_limit_overrides_state_check check (limit_state in ('unlimited', 'not_included', 'numeric')),
  add constraint organization_limit_overrides_state_value_check check (
    (limit_state = 'unlimited' and is_unlimited and limit_value is null)
    or (limit_state = 'not_included' and not is_unlimited and limit_value is null)
    or (limit_state = 'numeric' and not is_unlimited and limit_value is not null and limit_value >= 0)
  ),
  add constraint organization_limit_overrides_reason_check check (
    is_legacy_import or char_length(trim(coalesce(reason, ''))) between 1 and 1000
  ),
  add constraint organization_limit_overrides_actor_email_check check (
    actor_owner_email is null or char_length(trim(actor_owner_email)) between 3 and 320
  );

-- Extend the contractor-safe allowlist without permitting private reasons or owner identity.
...
-- Package changes use the existing append-only assignment table and add a corresponding private
-- commercial event and safe event in the same transaction.
...
revoke all on function public.apply_organization_package_change(uuid, uuid, text, text, text, timestamptz)
  from public, anon, authenticated;
grant execute on function public.apply_organization_package_change(uuid, uuid, text, text, text, timestamptz)
  to service_role;
...
revoke all on function public.apply_organization_feature_exception(uuid, text, text, timestamptz, timestamptz, text, text, text, timestamptz)
  from public, anon, authenticated;
grant execute on function public.apply_organization_feature_exception(uuid, text, text, timestamptz, timestamptz, text, text, text, timestamptz)
  to service_role;
...
revoke all on function public.apply_organization_limit_exception(uuid, text, text, integer, timestamptz, timestamptz, text, text, text, timestamptz)
  from public, anon, authenticated;
grant execute on function public.apply_organization_limit_exception(uuid, text, text, integer, timestamptz, timestamptz, text, text, text, timestamptz)
  to service_role;

comment on function public.apply_organization_package_change(uuid, uuid, text, text, text, timestamptz)
  is 'Immediately assigns a published package version and records immutable private and contractor-safe history.';
comment on function public.apply_organization_feature_exception(uuid, text, text, timestamptz, timestamptz, text, text, text, timestamptz)
  is 'Atomically changes a feature exception projection and records immutable reasoned history.';
