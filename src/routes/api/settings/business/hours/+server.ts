...
import { businessHoursSchema } from '$lib/server/validation/settings.schema';
...
export const PATCH: RequestHandler = async (event) => {
	...
	try {
		limit = await checkRateLimit(event.locals.supabase, {
			bucketKey: `settings-hours-save:${organizationId}`,
			...SAVE_LIMIT
		});
	} catch {
	...
	// Appointment only has no grid to send. The command clears the week rather than leaving times behind
	// that nothing displays any more.
	const { data, error } = await event.locals.supabase.rpc('save_organization_business_hours', {
		target_organization_id: organizationId,
		expected_revision: input.expected_revision,
		new_mode: input.mode,
		new_hours: input.mode === 'weekly' ? input.periods : null
	});
