-- Line-item images. Jobber's quote line-item card carries a photo per line (jobber-03-quotes.md §8.1); this
-- gives Request pricing rows the same, reusing the existing polymorphic attachments table and its presign/
-- upload/delete routes wholesale — nothing new to upload with, just a place to point at what was uploaded.

-- 1. attachments needs a tenant-safe target for other tables to reference ----------------------------------

alter table public.attachments
  add constraint attachments_organization_id_id_key unique (organization_id, id);

-- 2. Request pricing lines: a live, nullable reference ----------------------------------------------------

-- The browser uploads the photo (scoped to entity_type = 'request', entity_id = this request, using the
-- attachment routes that already exist) the moment it is picked, before the line is ever saved, the same way
-- name/price/qty are draft-only until Save. This column is what Save actually persists. A deleted attachment
-- degrades the still-editable line gracefully rather than blocking anything, so it is a real foreign key.
alter table public.request_pricing_lines
  add column image_attachment_id uuid;

alter table public.request_pricing_lines
  add constraint request_pricing_lines_image_organization_fk
  foreign key (organization_id, image_attachment_id)
  references public.attachments(organization_id, id) on delete set null;

create index request_pricing_lines_image_idx
  on public.request_pricing_lines(organization_id, image_attachment_id)
  where image_attachment_id is not null;

-- 3. Quote version lines: a frozen reference, not a live one ------------------------------------------------

-- Same rule this table already applies to source_catalog_item_id: a quote's copy must not change again once
-- taken, so a Request-side attachment deletion (from the Request's own, unrelated Attachments card) must
-- never reach into a quote's frozen line. No foreign key on purpose.
alter table public.quote_version_lines
  add column image_attachment_id uuid;

comment on column public.quote_version_lines.image_attachment_id is
  'Copied from request_pricing_lines at conversion. Deliberately not a foreign key, matching '
  'source_catalog_item_id: the attachment disappearing later must never reach into this frozen copy.';

-- 4. Teach the write path about the new column ---------------------------------------------------------------
...
-- 5. Carry the image across conversion, same as every other column -----------------------------------------
