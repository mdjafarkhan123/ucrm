-- Communications Part 3: 20260902140000_remove_client_owner_no_jobber_precedent.sql dropped
-- clients.owner_user_id, but this function still referenced it, so every real call has crashed with
-- 42703 since that migration landed. Nothing caught it because the worker stays fail-closed and no test
-- ever drove this exact branch -- the new pgTAP coverage for quote-sourced resend is what surfaced it.
-- An automated send has no acting person to prefer a sender for, so it always uses the organization's
-- default automated sender -- one of the two branches the removed OR condition already allowed.
