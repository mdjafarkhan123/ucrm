...
//
// This module is deliberately neutral. A quote signature and a job signature are the same act -- a name
// drawn on a screen -- and the only thing that differs between them is which prefix the PNG lands under,
// so the caller passes the key in rather than this file learning about either subject.
...
export async function storeSignatureImageAt(
	objectKey: string,
...
// Best effort on purpose. A failed cleanup must never turn a recorded signature into an error the person
