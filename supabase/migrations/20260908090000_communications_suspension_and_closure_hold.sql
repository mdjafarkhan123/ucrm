...
--
-- Organization suspension stops contractor-created outbound mail; inbound replies, unsubscribes,
-- complaints, delivery callbacks and reconciliation are untouched because none of them go through this
-- claim. Platform notices to the contractor (setup links, closure notices) send directly through Brevo
-- on their own path, so they keep flowing too.
--
-- Everything below is migration 20260906230000's claim body with one block added at the top of the
-- candidate loop. The claim is re-created rather than patched because plpgsql has no partial edit.
-- ---------------------------------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    -- both re-run every check in this loop from scratch, and 8.2's per-class deadlines, not this
    -- branch, decide what has grown too stale to release. Placement is first in the loop: it is the
    -- most decisive check, it is two index reads, and a stopped tenant should not have its recipient,
    -- suppression, sender or allowance state touched at all.
    --
    ...
    if hold_code is not null then
      ...
      set available_at = now() + interval '1 hour', last_error = hold_message
