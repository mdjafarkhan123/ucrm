...
// this request, which needs a session and the permission to see the job. The row is read through the
// member's own client, so row level security decides whether it exists at all — which is what narrows a
// Field member to the jobs they are on — and the job in the URL has to be the job the signature belongs
// to, so one job's link can never fetch another's.
