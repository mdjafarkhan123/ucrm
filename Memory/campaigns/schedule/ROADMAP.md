...
| 6a-2 | V1.1 | Add Schedule-owned lightweight Events | Complete — tested + browser-verified 2026-09-03 | Part 6a-1 | Passed: one-time **single-day** timed/Anytime whole-team Event created/edited/deleted only through the Schedule chooser Event tab + create-edit modal (no assignment, privacy, client/property or address fields); renders as its own type on Week/Day (Unassigned row)/Month with preview + Edit/Delete on all three; empty-slot seeds the slot, header "New" opens with no date and lets you pick the day; start-time-or-Anytime validation enforced; hard delete behind explicit "cannot be undone" confirm; create/edit/delete gated on jobs.schedule. Day/row/date count labels made kind-neutral ("item(s)") so events and assessments no longer read as visits |
...
| 7a | V1.2 | Provider-independent Map/routing foundation, built behind the boundary with deterministic mock fixtures | Planned — provider decided (managed Mapbox), building mock-first | Part 5; Part 6a; provider boundary approved 2026-09-03 | Route-order behavior (fixed-time + assessments locked as chronological anchors, only Anytime Visits draggable, saved order merged with current stops, shared Visit one stop per employee route), Google/Apple Directions deep-links incl. whole-route waypoint-limit message, stop-list UI, Map workspace shell (toggle, split, one-employee gating), all empty/filtered/failure states, provider boundary interface + mock geocoder, provisional geocode schema + on-save path. NOTHING requiring live tokens; geocode persistence NOT called complete here |
| 7b | V1.2 | Live managed-Mapbox integration + verification | Complete — 7b-A2 (live map/tiles) 2026-09-04, 7b-B (stored Permanent geocoding) 2026-09-04, both browser-verified | Part 7a; Mapbox account + server secret token (Permanent geocoding) + browser public token | Passed: real Mapbox Permanent geocoding backfilled all 8 pending properties (succeeded, stored lat/lng), worker route live (no longer 503), live tile/numbered-marker/route-line rendering confirmed in browser with a real stored coordinate (Savar, Dhaka pin + resolved address in the preview popover, no console errors). External navigation, rate-limit/provider-error paths and the full V1.2 completion journey are exercised structurally by 7a/7b-A2 but not re-walked end-to-end here — that is Part 8's job |
| 8a | Closure | Verify the V1 core dispatch journey end-to-end | **Complete — walked live + browser-verified 2026-09-04** | Parts 2–5 | Passed: all seven contract steps walked on one visit (Job #5). Undated create landed in the drawer (count 0→1 live); drag onto Week proposed "Not scheduled → Thu Sep 3 · 10am–11am" and saved; two assignees saved together (no dropped member); drag-move warned "Your business is closed that day" and saved; resize changed only the end (1:30→4:30pm); dragging onto another visit for a shared member warned "Jafar Khan is already booked then." **stacked with** the closed-day warning; Cancel restored the original; Mark complete raised the Jobs-owned "Final visit completed" chooser → Finish job; Job #5 turned "Requires invoicing" and its Visits card showed the identical "Sep 4, 2026 · 1:30 PM–4:30 PM / Completed Sep 4, 2026 / crew 2". Database confirmed the same row. **One defect found and fixed** (off-screen move confirmation — see below) |
| 8b | Closure | Verify the V1.1 calendar-variety and V1.2 Map journeys | Planned | Part 8a; Parts 6a, 7 | Assessments and Events carry their own meaning/actions/permissions; Map opens for one employee, stops readable, Anytime stops manually reordered + saved + surviving reload, unmappable address handled, Directions opened without losing Schedule state. Covers the three corners never exercised in Part 7: keyboard stop reordering, multi-pin route line, per-stop and whole-route Directions |
| 8c | Closure | Verify the cross-cutting edges and close the campaign | Planned | Parts 8a, 8b | A member without jobs.schedule gets no drag and no scheduling actions (absent or honestly disabled); recurring edits offer This visit only / This and later and honor the choice; a failed move restores the original and offers Retry; desktop keyboard access holds; a bounded large-window read reports truncation honestly |
...
## Popover anchoring for a drop point — Fixed 2026-09-04 (found by 8a)

An external drop (a backlog card dragged onto the grid) handed the move confirmation the whole day column as
its popover anchor. A column is a full 24 hours tall and mostly scrolled out of sight, so Floating UI placed
the confirmation at y = -59: everything above the buttons — the "Now / Move to" times and any warning — sat
off the top of the window, unreadable. An internal drag never showed this because it anchors to the card it
is moving. Both `probeExternal` implementations (Week's vertical columns and Day's horizontal employee
tracks) had it.

Fixed with Floating UI's virtual-element pattern, which is what it exists for: `anchorAtPoint(x, y)` in
`src/lib/components/ui/popover-anchor.ts` returns a `getBoundingClientRect`-only anchor at the drop point,
and `PopoverAnchor` widens `Popover`'s `anchor` prop so an element and a bare point are interchangeable.
`Popover`'s close-focus handler now guards with `instanceof HTMLElement`, since a point cannot take focus.

Correctness note for future changes: do not hand this popover a large or scrolled-out-of-view element as an
anchor. Collision padding cannot rescue an anchor rectangle that is itself mostly outside the viewport.
