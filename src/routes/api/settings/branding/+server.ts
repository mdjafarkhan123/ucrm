...
import { brandingSchema } from '$lib/server/validation/settings.schema';
import { zodFieldErrors } from '$lib/server/validation/foundation.schema';

const SAVE_LIMIT = { windowSeconds: 60, maxAttempts: 20 };

// The brand color. The logo has its own routes because it moves bytes as well as a row.
export const PATCH: RequestHandler = async (event) => {
	...
	try {
		limit = await checkRateLimit(event.locals.supabase, {
			bucketKey: `settings-branding-save:${organizationId}`,
			...SAVE_LIMIT
		});
	} catch {
	...
	const { data, error } = await event.locals.supabase.rpc('save_organization_branding', {
		target_organization_id: organizationId,
		expected_revision: parsed.data.expected_revision,
		new_brand_color: parsed.data.brand_color
	});
